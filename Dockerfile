...
RUN adduser -G root -m jenkins && \
  ...
  yum update -y && yum -y install java-1.8.0-openjdk python docker-client && \
  wget --no-check-certificate -P /home/jenkins/ =${SWARM_CLIENT_VERSION}/swarm-client-${SWARM_CLIENT_VERSION}.jar && \
